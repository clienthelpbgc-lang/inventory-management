...
import { TimestampCell } from "@/features/admin-tenants/components/timestamp-cell";
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          {tenant.lastSeenAt
            ? `Last seen ${daysAgoLabel(tenant.lastSeenAt, now).toLowerCase()}`
            : "Never signed in"}
          {` · Active ${tenant.activeDaysLast30} of the last 30 days`}
        </p>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold">Transactions, last 30 days</h2>

        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {tenant.transactionsByType.map((transaction) => (
            <div
              key={transaction.type}
              className="rounded-2xl border bg-white p-5 shadow-sm"
            >
              <p className="text-sm text-muted-foreground">
                {ACTIVITY_TYPE_LABELS[transaction.type]}
              </p>

              <p className="mt-2 text-3xl font-bold text-slate-900">
                {transaction.last30Days}
              </p>

              <p className="mt-1 text-sm text-muted-foreground">
                {transaction.lastAt
                  ? `Last used ${displayDate.format(new Date(transaction.lastAt))}`
                  : "Never used"}
              </p>
            </div>
          ))}
      ...
      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold">Transactions per week</h2>
        <p className="mt-1 mb-6 text-sm text-muted-foreground">
          Outgoings, incomings, returns and process orders created per week
        </p>

        <TenantActivityChart weeklyTransactions={tenant.weeklyTransactions} />
      </section>

      <section className="space-y-3">
        <h2 className="text-lg font-semibold">Users ({tenant.users.length})</h2>
              ...
              {tenant.users.length === 0 ? (
                ...
                tenant.users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell>{user.activeDaysLast30}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
